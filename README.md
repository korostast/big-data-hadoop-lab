...
Можно увидеть в UI Hadoop, что три воркера поднялись
![Hadoop UI](docs/spark-3-workers.png)
...
Мы провели несколько экспериментов с датасетом [NYC Yellow Taxi Trip Data](https://www.kaggle.com/datasets/elemento/nyc-yellow-taxi-trip-data) и Hadoop, а также с оптимизацией операций с помощью Spark. Если кратко:
1. Кэширование окупается не всегда. Обработка данных в RAM ускоряет тяжелые агрегации в разы, однако первоначальные затраты времени на сериализацию и shuffle (`.repartition().cache()`) делают этот подход невыгодным для простых или одноразовых ETL-пайплайнов. В общем такая оптимизация имеет смысл только при многократном переиспользовании датасета, что часто встречается в приложениях на практике (например, нам нужно провести ряд аналитический операций, чтобы исследовать данные)
    * В некоторых случаях Hadoop может втихаря сбросить данные на диск вместо того, чтобы полностью поместить их в RAM. За это отвечает параметр `spark.executor.memory`, который ограничивает доступ к RAM определённым лимитом размера RAM. Spark утилизирует лишь малую часть доступной оперативной памяти контейнера, что может нивелировать все преимущества кэширования
2. Конечно логично: масштабирование только HDFS (добавление DataNode) ускоряет дисковый ввод-вывод, но не решает проблему дорогих вычислений. Для реального ускорения агрегаций требуется синхронное добавление воркеров для обработки данных
3. Добавление вычислительных мощностей дает заметный прирост скорости, но он не является линейным. При переходе от 1 к 3 воркерам возникают издержки на пересылку партиций (shuffle опять же), которые снижают итоговую производительность
